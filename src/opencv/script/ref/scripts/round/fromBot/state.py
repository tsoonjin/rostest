    ...
    def execute(self, userdata):
        ...
        if self.pipe.red_img.detected and self.pipe.green_img.detected:
            if abs(self.pipe.red_img.deltaX) < 0.05:
                ...
                #Strike forward until it is near 
                while area < 7500:
                    if self.pipe.isAborted:
                        return 'aborted'
                    elif area < 5000:
                        ...
                        forward_mov = 0.5
                    self.pipe.sendMovement(forward=forward_mov, sidemove=0.0, wait=False)
                    sample = self.pipe.getAvg(self.pipe.red_img)
                    area_collection = [i.area for i in sample]
                    area = np.mean(area_collection)
                #Center the y 
                self.pipe.setDepth = self.pipe.currDepth + self.red_img.deltaY
                return 'locked'
...
class Moving(smach.State):
    ...
    def execute(self, userdata):
        while not self.pipe.isAborted:
            if not self.pipe.isCheck:
                rospy.loginfo("Moving")
                #Sidemove to get away from red pole
                while self.pipe.red_img.detected:
                    if self.pipe.red_img.incline == "left":
                        ...
                        self.pipe.sendMovement(sidemove=0.5, wait=False)
                rospy.loginfo("Done")
                self.pipe.isCheck = True
                return 'confirm'

            elif self.pipe.green_img.detected and not self.pipe.red_img.detected:
                #Avoided red pole, hovering just aboive green pole
                rospy.loginfo("BOLEH strike")
                return 'success'
            else:
                #Move backward to get wider field of vision
...
class Strike(smach.State):
    ...
    def execute(self, userdata):
        ...
        self.pipe.sendMovement(forward=strike)
        if self.pipe.isAborted:
            return 'aborted'
        if self.pipe.red_img.detected:
            return 'back'
