...
class Position(smach.State):
    ...
    def execute(self, userdata):
        ...
        while self.pipe.red_img.detected and self.pipe.green_img.detected:
            if abs(self.pipe.red_img.deltaX) < 0.05:
                rospy.loginfo("Centered, target acquired")
                while self.pipe.red_img.area < 8000:     
                    self.pipe.sendMovement(forward=2.0, wait=False)
                self.pipe.setDepth = self.pipe.currDepth + self.pipe.red_img.deltaY
                return 'locked'

            else:
                rospy.loginfo("Getting closer to the center")
                move_x = self.pipe.red_img.deltaX*self.con.mult_x
                self.pipe.sendMovement(sidemove=move_x, wait=False)
    ...
    def execute(self, userdata):
        if self.pipe.isAborted:
            ...
            while self.pipe.red_img.detected:
                ...
                elif self.pipe.red_img.incline == "right":
                    self.pipe.sendMovement(sidemove=0.5, wait=False)
                return 'success'

            if not self.pipe.red_img.detected and not self.pipe.green_img.detected:
...
class Strike(smach.State):
    ...
    def execute(self, userdata):
        ...
        self.pipe.sendMovement(forward=strike_d)
